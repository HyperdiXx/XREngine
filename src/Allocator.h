...
namespace allocator
{
	template <class T> T* allocateNew(Allocator& alloc)
	{
		return new (alloc.allocate(sizeof(T), __alignof(T))) T;
	}

	template <class T> T* allocateNew(Allocator& alloc, const T& t)
	{
		return new (alloc.allocate(sizeof(T), __alignof(T))) T(t);
	}

	template <class T> void deallocate(Allocator& alloc, T& obj)
	{
		obj.~T();
		alloc.deallocate(&obj);
	}

	template <class T> T* allocatorArray(Allocator& alloc, size_t length)
	{
		assert(length != 0);
	...
	template <class T> void deallocateArray(Allocator& alloc, T* array1)
	{
		assert(array1 != 0);
